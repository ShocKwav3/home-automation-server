...
const printDbQueryResultsString = consoleHelpers.colorize(consoleHelpers.colors.RED, 'printDbQueryResults(query)');
...
const sayInstructions = consoleHelpers.printOnConsole(`
  ...
  |
  |  Tip#1 Use ${printDbQueryResultsString} to print show result.
  ----------------------------------------------------------
`);
...
const sayBye = consoleHelpers.printOnConsole(`Moi Moi, ${user}!`);

const sayDbConClosing = consoleHelpers.printOnConsole(`${dbConClosingString}\n`);

const sayDbConError = (error) => consoleHelpers.printOnConsole(`${dbConErrorString}: ${error}\n`)();

const sayDbConOk = consoleHelpers.printOnConsole(`${dbConOkString}`);

const sayDbConVerifying = consoleHelpers.printOnConsole(`${dbConVerifyString}`);
