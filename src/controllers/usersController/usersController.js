...
const { user } = models;
const contextName = constants.controllerConstants.user.CONTEXTNAME;
...
const updateUser = (req, res) => {
    ...
    return user.update(requestedChanges, query)
        .then((userDataUpdateInformation) => {
            ...
            userControllerLog(
                `${logStylers.genericSuccess('User successfully updated! ')} Rows affected ${numberOfRowsAffected}`,
                `Incoming: ${logStylers.values(JSON.stringify(req.body))}`,
                `After change: ${logStylers.values(JSON.stringify(updatedUserData[0]))}`
            );
