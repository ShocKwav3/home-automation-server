...
const contextName = constants.controllerConstants.hub.CONTEXTNAME;
...
const updateHub = (req, res) => {
    ...
    return hub.update(requestedChanges, query)
        .then((hubDataUpdateInformation) => {
            ...
            hubsControllerLog(
                `${logStylers.genericSuccess('Hub successfully updated!')} Rows affected ${numberOfRowsAffected}`,
                `Incoming: ${logStylers.values(JSON.stringify(req.body))}`,
                `After change: ${logStylers.values(JSON.stringify(updatedHubData[0]))}`
            );
