...
const contextName = constants.controllerConstants.hub_profile.CONTEXTNAME;
...
const updateHubProfile = (req, res) => {
    ...
    return hub_profile.update(requestedChanges, query)
        .then((hubProfileDataUpdateInformation) => {
            ...
            hubProfileControllerLog(
                `${logStylers.genericSuccess('Hub profile successfully updated!')} Rows affected ${numberOfRowsAffected}`,
                `Incoming: ${logStylers.values(JSON.stringify(req.body))}`,
                `After change: ${logStylers.values(JSON.stringify(updatedHubProfileData[0]))}`
            );
